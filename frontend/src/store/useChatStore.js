...
export const useChatStore = create((set, get) => ({
  ...
  getUsers: async () => {
    try {
      const { unreadMessagesNumberMap, initializeUnreadMessagesNumberMap } =
        get();
  ...
  subscribeToMessages: () => {
    ...
    socket.on("newMessage", (newMessage) => {
      ...
      if (newMessage.senderId !== selectedUser?._id) {
        ...
        setUnreadMessagesNumberMap(
          newMessage.senderId,
          unreadMessagesNumberMap.get(newMessage.senderId) + 1
        );
        return;
      }
  ...
  // Function to initialize a unread message number map for all users
  ...
  // Function to update the unread messages number
